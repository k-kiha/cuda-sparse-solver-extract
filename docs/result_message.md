...
- 기본 경로는 cuSPARSE CSR SpMV, cuBLAS dot product, CUDA custom kernels, NVTX ranges를 포함합니다.
- optional iLU 경로는 CPU iLU(0) factorization과 GPU cuSPARSE SpSV triangular solve를 보여줍니다.
- optional AmgX 경로는 setup, solve, coefficient replacement/reuse 흐름을 보여줍니다.
...
4. optional iLU는 SpSV 구간이 전체 solver 시간을 얼마나 차지하는지 확인합니다.
5. optional AmgX는 setup 포함 solve와 coefficient reuse solve를 분리해 기록합니다.
