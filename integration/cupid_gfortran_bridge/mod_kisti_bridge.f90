...
contains
    subroutine mod_kisti_switch2(N, M, Arowp,Acoli,Aval, b)
        implicit none
        ...
        call cpu_time(ta0)
        GPUrowp(1:N+1) = Arowp(1:N+1)
        GPUcoli(1:M  ) = Acoli(1:M  ) 
        GPUv   (1:M  ) = Aval (1:M  )
        GPUb   (1:N  ) = b    (1:N  )
        ...
        call cpu_time(tc0)
        Arowp(1:N+1) = GPUrowp(1:N+1)
        Acoli(1:M  ) = GPUcoli(1:M  )
        Aval (1:M  ) = GPUv   (1:M  )
        b    (1:N  ) = GPUb   (1:N  )
        ierr = cudaDeviceSynchronize()
        call cpu_time(tc1)
    ...
    end subroutine mod_kisti_switch2
